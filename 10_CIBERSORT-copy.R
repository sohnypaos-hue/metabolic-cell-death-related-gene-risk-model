...
ssGSEA_data <- dat
gmtFile="/data/nas1/chenpeiru/44_GYZK-30212-7/10_CIBERSORT/mmc3.gmt"
#分析免疫细胞含量
#载入背景基因
...
write.csv(ssgsea_result1,"/data/nas1/refinement9/yanxiuhang/GYZK-30122-11/ssGSEA_result.csv",row.names = T,quote = F)
...
##############################################################################################
# 1️⃣ 计算每个 Cell_type 的 Wilcoxon p 值并做 BH 校正
pvals <- compare_means(Proportion ~ Type, data = box, group.by = "Cell_type",
...
# 3️⃣ 统一 y 坐标，让星号水平对齐
y_max <- max(box$Proportion) * 1.05
pvals$y <- y_max

# 4️⃣ 绘制 Boxplot
e1_box <- ggplot(box, aes(x = Cell_type, y = Proportion)) +
  geom_violin(aes(fill = Type), width = 0.6, position = position_dodge(0.5)) +
  geom_boxplot(aes(fill = Type), width = 0.2, alpha = 0.5, position = position_dodge(0.5)) +
  geom_text(data = pvals, aes(x = Cell_type, y = y, label = sig), inherit.aes = FALSE) +
...
# 保存 PDF 和 PNG
ggsave('/data/nas1/refinement9/yanxiuhang/GYZK-30122-11/box_ImmuneGeme_ssGSEA_FDR_aligned.pdf', plot = e1_box, width = 15, height = 6)
ggsave('/data/nas1/refinement9/yanxiuhang/GYZK-30122-11box_ImmuneGeme_ssGSEA_FDR_aligned.png', plot = e1_box, width = 15, height = 6, dpi = 300)

# 5️⃣ 绘制 Violin Plot
e1_violin <- ggplot(box, aes(x = Cell_type, y = Proportion)) +
  geom_violin(aes(fill = Type), width = 0.6, position = position_dodge(0.5)) +
  geom_boxplot(aes(fill = Type), width = 0.2, alpha = 0.5, position = position_dodge(0.5)) +
  geom_text(data = pvals, aes(x = Cell_type, y = y, label = sig), inherit.aes = FALSE) +
...
# 保存 PDF 和 PNG
ggsave('/data/nas1/refinement9/yanxiuhang/GYZK-30122-11violin_ImmuneGeme_ssGSEA_FDR_aligned.pdf', plot = e1_violin, width = 15, height = 6)
ggsave('/data/nas1/refinement9/yanxiuhang/GYZK-30122-11violin_ImmuneGeme_ssGSEA_FDR_aligned.png', plot = e1_violin, width = 15, height = 6, dpi = 300)
##################################################################################################
