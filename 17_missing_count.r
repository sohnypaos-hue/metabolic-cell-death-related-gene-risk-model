...
## 读取从xena下载的数据
tcga.expr1 <- read_tsv(file = './GYZK-30122-11/44_GYZK-30212-7/00_raw_data/01_TCGA/TCGA-BLCA.htseq_counts.tsv')%>%lc.tableToNum()#Rows: 60488 Columns: 431 
head(tcga.expr1)
...
## 对数据进行id转化
genecode<-read.table(file = './GYZK-30122-11/44_GYZK-30212-7/00_raw_data/01_TCGA/gencode.v22.annotation.gene.probeMap')
...
# 411
## 保留有生存数据的
survival<-read.delim2('./GYZK-30122-11/44_GYZK-30212-7/00_raw_data/01_TCGA/TCGA-BLCA.survival.tsv')#448,4

write.table(survival,file = './GYZK-30122-11/44_GYZK-30212-7/00_raw_data/01_TCGA/survival.xls',sep = '\t',row.names = F,quote = F)
head(survival)
exp_tumor<-exp_tumor[,colnames(exp_tumor)%in%survival$sample]
#406
head(exp_tumor)
pcg <- read.delim2('./GYZK-30122-11/PCG.xls(v22)')
...
write.table(dat.final,file = './GYZK-30122-11/dat.tcga_count.xls',sep = '\t',quote = F,row.names = T)
...
##fpkm
setwd("./GYZK-30122-11/44_GYZK-30212-7/00_raw_data/01_TCGA/")
...
# -----------------------------
# 除样本ID外的列
clinical_raw <- phenotype[, -1]

# 缺失率函数
missing_ratio <- function(x) sum(is.na(x)) / length(x)
missing_count <- function(x) sum(is.na(x))

# 计算缺失比例和缺失数量
clinical_missing <- sapply(clinical_raw, missing_ratio)
clinical_missing_count <- sapply(clinical_raw, missing_count)

# 整理成数据框
missing_summary <- data.frame(
  Feature = names(clinical_missing),
  Missing_Rate = round(clinical_missing, 4),
...
# 保存到文件
write.csv(missing_summary, file = './../../../clinical_missing_summary.csv', row.names = FALSE)
####################################################################################################
